...
#   *** (next line exports datasets with nested_items in flat NDJSON format) ***
#   RAILS_ENV=demo bin/rails migration:legacy:export_bundle OUTPUT_ROOT=/tmp/databank_exports
#   *** (next line includes an optional dedicated nested_items export) ***
#   RAILS_ENV=demo bin/rails migration:legacy:export_nested_items_bundle OUTPUT_ROOT=/tmp/databank_exports
...
#      RAILS_ENV=demo SEPARATE_NESTED_ITEMS_BUNDLE=true bin/rails migration:legacy:export_bundle OUTPUT_ROOT=/tmp/databank_exports # rubocop:disable Layout/LineLength
#   2) Export nested items as a separate bundle (nested_items only):
#      RAILS_ENV=demo SEPARATE_NESTED_ITEMS_BUNDLE=true bin/rails migration:legacy:export_nested_items_bundle OUTPUT_ROOT=/tmp/databank_exports # rubocop:disable Layout/LineLength
#
# Recommended full export command (runs all bundles including both dataset steps above):
#   RAILS_ENV=demo SEPARATE_NESTED_ITEMS_BUNDLE=true bin/rails migration:legacy:export_all OUTPUT_ROOT=/tmp/databank_exports # rubocop:disable Layout/LineLength
#
# Runbook (export side):
  ...
  def as_json # rubocop:disable Metrics/MethodLength,Metrics/AbcSize
    {
      ...
      corresponding_creator_email: dataset.corresponding_creator_email,
      ...
      all_globus:                 dataset.all_globus,
      all_medusa:                 dataset.all_medusa,
      complete:                   dataset.complete,
      org_creators:               dataset.org_creators,
      external_files_link:        dataset.external_files_link,
      external_files_note:        dataset.external_files_note,
      medusa_dataset_dir:         dataset.medusa_dataset_dir,
      suppress_changelog:         dataset.suppress_changelog,
      version_comment:            dataset.version_comment,
      have_permission:            dataset.have_permission,
      removed_private:            dataset.removed_private,
      agree:                      dataset.agree,
...
class Migration::Legacy::AuditExportSerializer
  ...
  def as_json # rubocop:disable Metrics/AbcSize
    {
      ...
      attributes: {
        ...
        auditable:       serialized_reference(type: audit.auditable_type, legacy_id: audit.auditable_id,
                                              record: audit.auditable),
        associated:      serialized_reference(type: audit.associated_type, legacy_id: audit.associated_id,
                                              record: audit.associated)
      }
    }
  end
  ...
  def serialized_user_identity
    return nil if audit.user_id.blank?
  ...
  def locator_for(type:, record:) # rubocop:disable Metrics/MethodLength
    case type
    when "Dataset"
      {
        key: dataset.key
  ...
  def attribute_value(record:, key:, fallback_keys: [])
    ([key] + fallback_keys)
      .lazy
      .map {|candidate_key| value_for_key(record: record, key: candidate_key) }
      .find(&:present?)
  end

  def value_for_key(record:, key:)
    return record.public_send(key) if record.respond_to?(key)

    key_from_changes(key)
  end
...
class Migration::Legacy::GuideExportSerializer
  ...
  def serialized_attributes # rubocop:disable Metrics/AbcSize
    base = {
      id:         record.id,
...
class Migration::Legacy::DatasetAccessGrantExportSerializer
  def initialize(dataset_key:, email:, access_level:, user_provider:, user_uid:, legacy_resource_id:, abilities:) # rubocop:disable Metrics/ParameterLists
...
class Migration::Legacy::FlatExportSerializer
  def self.serialize_dataset(dataset) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
    {
      type:                       "dataset",
  ...
  def self.serialize_related_materials(materials) # rubocop:disable Metrics/AbcSize,Metrics/CyclomaticComplexity,Metrics/PerceivedComplexity
    materials.order(:id).map do |material|
      ...
      material_type = material.material_type.presence || material.selected_type.presence || "material"
      material_title = material.citation.presence || material.link.presence || material.uri.presence || material_type
      {
...
namespace :migration do # rubocop:disable Metrics/BlockLength
  namespace :legacy do # rubocop:disable Metrics/BlockLength
    ...
    def users_export_paths(output_root:)
    ...
    def collect_user_export_data
...

      {
        ...
        counts_by_mapped_role: counts_by_mapped_role,
        skipped_counts:        skipped_counts
      }
    end

    def write_ndjson_bundle(records:, bundle_path:, bundle_file:, checksum_path:)
    ...
    def write_users_manifest(manifest_path:, bundle_file:, records:, counts_by_role:, counts_by_mapped_role:, # rubocop:disable Metrics/ParameterLists
                             skipped_counts:, checksum:)
      manifest = {
    ...
    def print_users_export_summary(bundle_path:, checksum_path:, manifest_path:, records:, skipped_counts:)
    ...
    def featured_researchers_export_filters
    ...
    def featured_researchers_export_paths(output_root:)
      timestamp = Time.current.utc.strftime("%Y%m%dT%H%M%SZ")
    ...
    def featured_researchers_scope(active_only:, since_time:, until_time:)
    ...
    def write_featured_researchers_bundle(scope:, bundle_path:)
      digest = Digest::SHA256.new
      count = 0
    ...
    def write_featured_researchers_manifest(manifest_path:, bundle_file:, count:, checksum:, active_only:, since_time:, until_time:) # rubocop:disable Metrics/ParameterLists,Layout/LineLength
      manifest = {
        generated_at:   Time.current.utc.iso8601,
        bundle_file:    bundle_file,
        record_count:   count,
    ...
    def print_featured_researchers_export_summary(bundle_path:, checksum_path:, manifest_path:, count:)
      puts "Bundle: #{bundle_path}"
      puts "Checksum: #{checksum_path}"
      puts "Manifest: #{manifest_path}"
    ...
    def permissions_export_paths(output_root:)
      timestamp = Time.current.utc.strftime("%Y%m%dT%H%M%SZ")
    ...
    def normalize_permission_email(value:)
    ...
    def resolve_permission_email(ability:)
      user = User.find_by(provider: ability.user_provider, uid: ability.user_uid)
      candidate = user&.email.presence || ability.user_uid
      normalized = normalize_permission_email(value: candidate)
      return nil unless normalized =~ URI::MailTo::EMAIL_REGEXP

      normalized
    end

    def append_permission_records(rows:, type:, records:, skipped:)
      rows.find_each do |ability|
        email = resolve_permission_email(ability: ability)
        if email.blank?
          skipped += 1
          next
        end
    ...
    def collect_permission_export_data
      ...
      deposit_exception_rows = UserAbility.where(resource_type: "Dataset", resource_id: nil,
                                                 ability: "create").order(:id)
      skipped = append_permission_records(
        rows:    deposit_exception_rows,
    ...
    def write_permissions_manifest(manifest_path:, bundle_file:, unique_records:, counts:, skipped:, checksum:) # rubocop:disable Metrics/ParameterLists
      manifest = {
        generated_at:    Time.current.utc.iso8601,
        bundle_file:     bundle_file,
    ...
    def print_permissions_export_summary(bundle_path:, checksum_path:, manifest_path:, unique_records:, counts:, skipped:) # rubocop:disable Metrics/ParameterLists,Layout/LineLength
      puts "Bundle: #{bundle_path}"
      puts "Checksum: #{checksum_path}"
      puts "Manifest: #{manifest_path}"
    ...
    def dataset_access_grants_export_paths(output_root:)
      timestamp = Time.current.utc.strftime("%Y%m%dT%H%M%SZ")
    ...
    def grouped_dataset_access_grant_rows
      UserAbility
        .where(resource_type: "Dataset")
        .where.not(resource_id: nil)
        .where(ability: ["read", "view_files", "update"])
    ...
    def collect_dataset_access_grant_export_data
      records = []
      skipped = 0

      grouped_dataset_access_grant_rows.each_value do |abilities_for_user|
        record = dataset_access_grant_record_for(abilities_for_user: abilities_for_user)
        if record.nil?
          skipped += 1
          next
        end
      ...
      summary = summarize_dataset_access_grant_records(records: records)
    ...
    def dataset_access_grant_record_for(abilities_for_user:)
      exemplar = abilities_for_user.first
      dataset = Dataset.find_by(id: exemplar.resource_id)
      return nil if dataset.nil? || dataset.key.blank?

      email = resolve_permission_email(ability: exemplar)
      ...
      Migration::Legacy::DatasetAccessGrantExportSerializer.new(
        dataset_key:        dataset.key,
        email:              email,
        access_level:       access_level,
    ...
    def summarize_dataset_access_grant_records(records:)
    ...
    def write_dataset_access_grants_manifest(manifest_path:, bundle_file:, unique_records:, counts:, skipped:, checksum:) # rubocop:disable Metrics/ParameterLists,Layout/LineLength
      manifest = {
        generated_at:    Time.current.utc.iso8601,
        bundle_file:     bundle_file,
        record_count:    unique_records.count,
    ...
    def print_dataset_access_grants_export_summary(bundle_path:, checksum_path:, manifest_path:, unique_records:, counts:, skipped:) # rubocop:disable Metrics/ParameterLists,Layout/LineLength
      puts "Bundle: #{bundle_path}"
      puts "Checksum: #{checksum_path}"
      puts "Manifest: #{manifest_path}"
      puts "Record count: #{unique_records.count}"
    ...
    def medusa_ingests_export_filters
      since_raw = ENV["SINCE"]
      until_raw = ENV["UNTIL"]
    ...
    def medusa_ingests_export_paths(output_root:)
      timestamp = Time.current.utc.strftime("%Y%m%dT%H%M%SZ")
    ...
    def medusa_ingests_scope(since_time:, until_time:)
    ...
    def collect_medusa_ingest_export_data(scope:)
      records = []
      skipped = 0
    ...
    def summarize_medusa_ingest_records(records:)
      records
        .group_by {|record| record.dig(:attributes, :request_status).to_s.presence || "pending" }
        .transform_values(&:count)
    end

    def write_medusa_ingests_manifest(manifest_path:, bundle_file:, records:, counts:, skipped:, checksum:) # rubocop:disable Metrics/ParameterLists
      manifest = {
        generated_at:    Time.current.utc.iso8601,
        bundle_file:     bundle_file,
        record_count:    records.count,
    ...
    def print_medusa_ingests_export_summary(bundle_path:, checksum_path:, manifest_path:, records:, skipped:, counts:) # rubocop:disable Metrics/ParameterLists
      puts "Bundle: #{bundle_path}"
      puts "Checksum: #{checksum_path}"
      puts "Manifest: #{manifest_path}"
      puts "Record count: #{records.count}"
    ...
    def download_metrics_export_filters
    ...
    def download_metrics_export_paths(output_root:)
      timestamp = Time.current.utc.strftime("%Y%m%dT%H%M%SZ")
    ...
    def download_metrics_scopes(include_tests:, since_time:, until_time:)
    ...
    def append_download_metric_rows(scope:, serializer_class:, count_key:, file:, digest:, counts:) # rubocop:disable Metrics/ParameterLists
      appended = 0

      scope.find_each(batch_size: 1000) do |row|
        line = JSON.generate(serializer_class.new(row).as_json)
        file.write(line)
        file.write("\n")
        digest.update(line)
        digest.update("\n")
    ...
    def write_download_metrics_bundle(scopes:, bundle_path:) # rubocop:disable Metrics/MethodLength
      digest = Digest::SHA256.new
      counts = Hash.new(0)
      record_count = 0

      File.open(bundle_path, "w") do |file|
        record_count += append_download_metric_rows(
          scope:            scopes[:dataset_scope],
          serializer_class: Migration::Legacy::DatasetDownloadTallyExportSerializer,
          count_key:        "DatasetDownloadTally",
          file:             file,
          digest:           digest,
          counts:           counts
        )
        ...
        record_count += append_download_metric_rows(
          ...
          serializer_class: Migration::Legacy::FileDownloadTallyExportSerializer,
          count_key:        "FileDownloadTally",
          file:             file,
          digest:           digest,
          counts:           counts
        )
        ...
        record_count += append_download_metric_rows(
          scope:            scopes[:day_scope],
          serializer_class: Migration::Legacy::DayFileDownloadExportSerializer,
          count_key:        "DayFileDownload",
          file:             file,
          digest:           digest,
          counts:           counts
        )
    ...
    def write_download_metrics_manifest(manifest_path:, bundle_file:, result:, include_tests:, since_time:, until_time:) # rubocop:disable Metrics/ParameterLists
      manifest = {
        generated_at:   Time.current.utc.iso8601,
        bundle_file:    bundle_file,
        record_count:   result[:record_count],
        counts:         {
          "DatasetDownloadTally" => result[:counts].fetch("DatasetDownloadTally", 0),
          "FileDownloadTally"    => result[:counts].fetch("FileDownloadTally", 0),
          "DayFileDownload"      => result[:counts].fetch("DayFileDownload", 0)
        },
    ...
    def print_download_metrics_export_summary(bundle_path:, checksum_path:, manifest_path:, result:)
      puts "Bundle: #{bundle_path}"
      puts "Checksum: #{checksum_path}"
      puts "Manifest: #{manifest_path}"
      puts "Record count: #{result[:record_count]}"
      puts "DatasetDownloadTally records: #{result[:counts].fetch('DatasetDownloadTally', 0)}"
      puts "FileDownloadTally records: #{result[:counts].fetch('FileDownloadTally', 0)}"
      puts "DayFileDownload records: #{result[:counts].fetch('DayFileDownload', 0)}"
    end
    ...
    task export_users_bundle: :environment do
      ...
      paths = users_export_paths(output_root: output_root)
      export_data = collect_user_export_data

      checksum = write_ndjson_bundle(
      ...
      write_users_manifest(
        manifest_path:         paths[:manifest_path],
        bundle_file:           paths[:bundle_file],
        records:               export_data[:records],
        counts_by_role:        export_data[:counts_by_role],
        counts_by_mapped_role: export_data[:counts_by_mapped_role],
        skipped_counts:        export_data[:skipped_counts],
        checksum:              checksum
      )

      print_users_export_summary(
        bundle_path:    paths[:bundle_path],
        checksum_path:  paths[:checksum_path],
        manifest_path:  paths[:manifest_path],
        records:        export_data[:records],
    ...
    task export_guides_bundle: :environment do # rubocop:disable Metrics/BlockLength
      ...
      File.open(bundle_path, "w") do |file| # rubocop:disable Metrics/BlockLength
        sections = scope.to_a.sort_by {|section| [section.ordinal || Float::INFINITY, section.id] }

        sections.each do |section| # rubocop:disable Metrics/BlockLength
    ...
    task export_featured_researchers_bundle: :environment do
      ...
      filters = featured_researchers_export_filters
      paths = featured_researchers_export_paths(output_root: output_root)
      scope = featured_researchers_scope(
        active_only: filters[:active_only],
        since_time:  filters[:since_time],
        until_time:  filters[:until_time]
      )
      result = write_featured_researchers_bundle(scope: scope, bundle_path: paths[:bundle_path])

      File.write(paths[:checksum_path], "#{result[:checksum]}  #{paths[:bundle_file]}\n")

      write_featured_researchers_manifest(
        manifest_path: paths[:manifest_path],
        bundle_file:   paths[:bundle_file],
        count:         result[:count],
      ...
      print_featured_researchers_export_summary(
        bundle_path:   paths[:bundle_path],
        checksum_path: paths[:checksum_path],
        manifest_path: paths[:manifest_path],
        count:         result[:count]
    ...
    task export_audits_bundle: :environment do # rubocop:disable Metrics/BlockLength
      ...
      File.open(bundle_path, "w") do |file|
        scope.find_each(batch_size: 100) do |dataset|
          ...
          audits.sort_by {|audit| [audit.created_at || Time.zone.at(0), audit.id] }.each do |audit|
    ...
    task export_permissions_bundle: :environment do
      ...
      paths = permissions_export_paths(output_root: output_root)
      export_data = collect_permission_export_data
      checksum = write_ndjson_bundle(
        records:       export_data[:unique_records],
        bundle_path:   paths[:bundle_path],
        bundle_file:   paths[:bundle_file],
        checksum_path: paths[:checksum_path]
      )
      ...
      write_permissions_manifest(
        manifest_path:  paths[:manifest_path],
        bundle_file:    paths[:bundle_file],
        unique_records: export_data[:unique_records],
        counts:         export_data[:counts],
        skipped:        export_data[:skipped],
        checksum:       checksum
      )

      print_permissions_export_summary(
        bundle_path:    paths[:bundle_path],
        checksum_path:  paths[:checksum_path],
        manifest_path:  paths[:manifest_path],
        unique_records: export_data[:unique_records],
    ...
    task export_dataset_access_grants_bundle: :environment do
      ...
      paths = dataset_access_grants_export_paths(output_root: output_root)
      export_data = collect_dataset_access_grant_export_data
      checksum = write_ndjson_bundle(
        records:       export_data[:unique_records],
        bundle_path:   paths[:bundle_path],
        bundle_file:   paths[:bundle_file],
      ...
      write_dataset_access_grants_manifest(
        manifest_path:  paths[:manifest_path],
        bundle_file:    paths[:bundle_file],
        unique_records: export_data[:unique_records],
        counts:         export_data[:counts],
      ...
      print_dataset_access_grants_export_summary(
        bundle_path:    paths[:bundle_path],
        checksum_path:  paths[:checksum_path],
        manifest_path:  paths[:manifest_path],
        unique_records: export_data[:unique_records],
    ...
    task export_medusa_ingests_bundle: :environment do # rubocop:disable Metrics/BlockLength
      output_root = ENV.fetch("OUTPUT_ROOT", Rails.root.join("tmp/migration_exports_medusa_ingests").to_s)

      filters = medusa_ingests_export_filters
      paths = medusa_ingests_export_paths(output_root: output_root)
      scope = medusa_ingests_scope(since_time: filters[:since_time], until_time: filters[:until_time])
      export_data = collect_medusa_ingest_export_data(scope: scope)
      counts = summarize_medusa_ingest_records(records: export_data[:records])
      checksum = write_ndjson_bundle(
        records:       export_data[:records],
        bundle_path:   paths[:bundle_path],
        bundle_file:   paths[:bundle_file],
      ...
      print_medusa_ingests_export_summary(
        bundle_path:   paths[:bundle_path],
        checksum_path: paths[:checksum_path],
        manifest_path: paths[:manifest_path],
        records:       export_data[:records],
    ...
    task export_download_metrics_bundle: :environment do
      ...
      filters = download_metrics_export_filters
      paths = download_metrics_export_paths(output_root: output_root)
      scopes = download_metrics_scopes(
        include_tests: filters[:include_tests],
        since_time:    filters[:since_time],
        until_time:    filters[:until_time]
      )
      result = write_download_metrics_bundle(scopes: scopes, bundle_path: paths[:bundle_path])

      File.write(paths[:checksum_path], "#{result[:checksum]}  #{paths[:bundle_file]}\n")

      write_download_metrics_manifest(
        manifest_path: paths[:manifest_path],
        bundle_file:   paths[:bundle_file],
        result:        result,
        include_tests: filters[:include_tests],
      ...
      print_download_metrics_export_summary(
        bundle_path:   paths[:bundle_path],
        checksum_path: paths[:checksum_path],
        manifest_path: paths[:manifest_path],
        result:        result
    ...
    task export_bundle: :environment do # rubocop:disable Metrics/BlockLength
      ...
      File.open(bundle_path, "w") do |file| # rubocop:disable Metrics/BlockLength
        scope.find_each(batch_size: 50) do |dataset| # rubocop:disable Metrics/BlockLength
          ...
          dataset.datafiles.each do |datafile|
            ...
            next unless export_nested_inline
          ...
          if (dataset_count % 10).zero?
            ...
            puts "[#{percent}%] #{dataset_count}/#{total_datasets} datasets, #{datafile_count} datafiles, #{nested_item_count} items (#{elapsed}s elapsed, ~#{remaining}s remaining)" # rubocop:disable Layout/LineLength
          end
        end
      end
    ...
    def test_bundle_export_paths(output_root:)
      timestamp = Time.current.utc.strftime("%Y%m%dT%H%M%SZ")
    ...
    def seed_test_dataset_keys
      seed_keys = ["TESTIDB-MIGRATE1", "TESTIDB-MIGRATE2", "TESTIDB-MIGRATE3"]
      seed_datasets = Dataset.where(key: seed_keys).order(:key)
      return nil unless seed_datasets.count == seed_keys.size

      puts "Using seed migration test datasets: #{seed_keys.join(', ')}"
      seed_keys
    end

    def scan_test_dataset_keys_with_nested_items(scope:)
      test_keys = []
      scope.find_each do |dataset|
        next unless dataset.datafiles.any? {|df| df.nested_items.any? }
    ...
    def fallback_test_dataset_keys(scope:)
      scope.limit(3).pluck(:key)
    end

    def resolve_test_dataset_keys(explicit_keys:)
      if explicit_keys.present?
        puts "Using explicit KEYS: #{explicit_keys.join(', ')}"
        return explicit_keys
      end

      test_keys = seed_test_dataset_keys
      return test_keys if test_keys.present?

      scope = Dataset.includes(:creators, :contributors, :funders, :related_materials, :datafiles, :notes)
                     .where(is_test: false)
                     .order(:id)

      test_keys = scan_test_dataset_keys_with_nested_items(scope: scope)
      test_keys = fallback_test_dataset_keys(scope: scope) if test_keys.length < 3

      test_keys
    end

    def write_test_bundle(scope:, bundle_path:) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
      digest = Digest::SHA256.new
      dataset_count = 0
      datafile_count = 0
      nested_item_count = 0
    ...
    def write_test_bundle_manifest(manifest_path:, bundle_file:, result:)
      manifest = {
        generated_at:   Time.current.utc.iso8601,
        bundle_file:    bundle_file,
        format_version: 2,
        ...
        record_counts:  {
          ...
          datafiles:    result[:datafile_count],
    ...
    def print_test_bundle_export_summary(bundle_path:, checksum_path:, manifest_path:, result:)
      puts "\n✓ Flat test bundle created!"
      puts "Bundle: #{bundle_path}"
      puts "Checksum: #{checksum_path}"
      puts "Manifest: #{manifest_path}"
      puts "Datasets: #{result[:dataset_count]}"
      puts "Datafiles: #{result[:datafile_count]}"
      puts "Nested items: #{result[:nested_item_count]}"
    end
    ...
    task export_test_bundle: :environment do
      ...
      paths = test_bundle_export_paths(output_root: output_root)
      test_keys = resolve_test_dataset_keys(explicit_keys: explicit_keys)
      ...
      result = write_test_bundle(scope: scope, bundle_path: paths[:bundle_path])

      File.write(paths[:checksum_path], "#{result[:checksum]}  #{paths[:bundle_file]}\n")

      write_test_bundle_manifest(
        manifest_path: paths[:manifest_path],
        bundle_file:   paths[:bundle_file],
        result:        result
      )

      print_test_bundle_export_summary(
        bundle_path:   paths[:bundle_path],
        checksum_path: paths[:checksum_path],
        manifest_path: paths[:manifest_path],
        result:        result
