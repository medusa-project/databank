...
class FeaturedResearchersController < ApplicationController
  ...
  # Responds to `GET /featured_researchers`
  # Responds to `GET /featured_researchers.json`
  def index
  ...
  # Responds to `PATCH/PUT /featured_researchers/1`
  # Responds to `PATCH/PUT /featured_researchers/1.json`
  def update
    respond_to do |format|
  ...
  # Responds to `DELETE /featured_researchers/1`
  # Responds to `DELETE /featured_researchers/1.json`
  def destroy
    @featured_researcher.destroy
    respond_to do |format|
      format.html {
        redirect_to featured_researchers_url, notice: 'Featured researcher entry was successfully destroyed.'
      }
  ...
  def featured_researcher_params
    params.require(:featured_researcher).permit(:name, :question, :dataset_url, :article_url, :bio, :testimonial,
                                                :photo_url, :is_active)
  end
