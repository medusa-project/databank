...
class MedusaIngestsController < ApplicationController
  ...
  # Responds to `GET /medusa_ingests`
  # Responds to `GET /medusa_ingests.json`
  def index
    @medusa_ingests = MedusaIngest.order(created_at: :desc)
  end

  # Responds to `GET /medusa_ingests/1`
  # Responds to `GET /medusa_ingests/1.json`
  def show
    @ingest_responses = IngestResponse.where(staging_key: @medusa_ingest.staging_key)
  end

  # Responds to `GET /medusa_ingests/new`
  def new
    @medusa_ingest = MedusaIngest.new
  end

  # Responds to `GET /medusa_ingests/1/edit`
  def edit; end

  # Responds to `POST /medusa_ingests`
  # Responds to `POST /medusa_ingests.json`
  def create
  ...
  # Responds to `PATCH/PUT /medusa_ingests/1`
  # Responds to `PATCH/PUT /medusa_ingests/1.json`
  def update
    respond_to do |format|
  ...
  # Responds to `DELETE /medusa_ingests/1`
  # Responds to `DELETE /medusa_ingests/1.json`
  def destroy
