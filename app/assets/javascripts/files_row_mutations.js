(function (window) {
  ...
  Databank.files.rowMutations = {
    removeFileRowPreConfirm: function (datafile_index) {
      if (
        jQuery(
          "#dataset_datafiles_attributes_" + datafile_index + "_web_id",
        ).val() == undefined
      ) {
        ...
        jQuery.ajax({
          ...
          success: function () {
            ...
            jQuery(
              "#dataset_datafiles_attributes_" + datafile_index + "_id",
            ).remove();
          },
          error: function (xhr) {
            var errorMessage = "Unable to remove file. Please try again.";

            try {
              var err = JSON.parse(xhr.responseText);
              errorMessage =
                err.Message || err.message || err.error || errorMessage;
            } catch (e) {
              console.log("Failed to parse remove file error response", e);
            }
    ...
    removeFileJobRow: function (job_id, datafile_id) {
      if (window.confirm("Are you sure?")) {
        ...
        var row =
          ...
          newId +
          '"><td>' +
          '<input value="true" type="hidden" name="dataset[datafiles_attributes][' +
          newId +
          ...
          '<input value="' +
          datafile_id +
          '" type="hidden" name="dataset[datafiles_attributes][' +
          newId +
          '][_id]" id="dataset_datafiles_attributes_' +
          newId +
          '_id "/>' +
          "</td></tr>";
    ...
    downloadSelected: function () {
      var file_ids = jQuery("input[name='selected_files[]']:checked").map(
      ...
      jQuery.each(file_ids, function (i, file_id) {
        var fileURL =
          "<iframe class='hidden' src='/datasets/" +
          dataset_key +
          "/stream_file/" +
          file_id +
          "'></iframe>";
    ...
    appendFileRow: function (newFile) {
      ...
      var row =
        ...
        newId +
        '][web_id]" id="dataset_datafiles_attributes_' +
        newId +
        '_web_id" />' +
        '<input type="hidden"  value="' +
        file.datafileId +
