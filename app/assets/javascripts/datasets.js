...
ready = function () {
  ...
  jQuery(document)
    .off("click.datasetActions", "[data-dataset-action]")
    .on("click.datasetActions", "[data-dataset-action]", function (event) {
      var action = jQuery(this).data("dataset-action");

      switch (action) {
        case "suppress-changelog":
          event.preventDefault();
          suppressChangelog();
          break;
        case "unsuppress-changelog":
          event.preventDefault();
          unsuppressChangelog();
          break;
        case "tmp-suppress-files":
          event.preventDefault();
          tmpSuppressFiles();
          break;
        case "tmp-suppress-metadata":
          event.preventDefault();
          tmpSuppressMetadata();
          break;
        case "unsuppress":
          ...
          unsuppress();
          break;
        case "perm-suppress-files":
          event.preventDefault();
          permSuppressFiles();
          break;
        case "perm-suppress-metadata":
          event.preventDefault();
          permSuppressMetadata();
          break;
        case "unsuppress-review":
          event.preventDefault();
          unsuppressReview();
          break;
        case "suppress-review":
          event.preventDefault();
          suppressReview();
          break;
        case "version-to-draft":
          event.preventDefault();
          version2draft();
          break;
        case "draft-to-version":
          event.preventDefault();
          draft2version();
          break;
        default:
  ...
  jQuery(".more").each(function () {
    ...
    if (content.length > showChar) {
      ...
      var html =
        `${visibleContent}<span class="moreellipses">${ellipsesText}&nbsp;</span>` +
  ...
  jQuery("#new-exit-button").click(function () {
    ...
    if (
      $form.find("input[type='hidden'][name='context'][value='exit']")
        .length === 0
    ) {
  ...
  jQuery("#api-modal-btn").click(function () {
    jQuery.ajax({
      ...
      error: function () {
        alert(
          "We're sorry, something went wrong when retrieving your token. Please try again or contact support.",
        );
      },
    });
  ...
  jQuery("#reserve-doi-btn").click(function () {
    jQuery.getJSON(
      "/datasets/" + dataset_key + "/reserve_doi",
      function (data) {
        if (data.status && data.status == "ok") {
          jQuery("#deposit").modal("show");

          jQuery("#your-doi-here").html(
            "We've reserved a DOI for you: " +
              data.doi +
              ", but your dataset is not yet published.",
          );
        } else {
          alert(
            "We're sorry, something went wrong during an attempt to reserve a DOI for this dataset.",
          );
        }
      },
    );
...
function handleAgreeModal(email, name) {
  Databank.agreement.handleModal(email, name);
}

function handlePrivateYes() {
  Databank.agreement.handlePrivateYes();
}

function handlePrivateNA() {
  Databank.agreement.handlePrivateNA();
}

function handlePrivateNo() {
  Databank.agreement.handlePrivateNo();
}
...
function handleOwnerYes() {
  Databank.agreement.handleOwnerYes();
}

function handleOwnerNo() {
  Databank.agreement.handleOwnerNo();
}

function handleAgreeYes() {
  Databank.agreement.handleAgreeYes();
}

function handleAgreeNo() {
  Databank.agreement.handleAgreeNo();
}

function agree_answers_all_yes() {
  return Databank.agreement.allAnswersYes();
}

function agree_answers_none_no() {
  return Databank.agreement.noneAnswersNo();
}

function allow_agree_submit() {
  Databank.agreement.allowSubmit();
}

// Previously undefined — now delegates to Databank.agreement.handleNotAgreed.
function handleNotAgreed() {
  Databank.agreement.handleNotAgreed();
}
...
function validateReleaseDate() {
  ...
  if (releaseDate > yearFromNow) {
    alert(
      "The maximum amount of time that data can be delayed for publication is is 1 year.",
    );
    jQuery("#dataset_release_date").val(
      yearFromNow.getFullYear() +
        "-" +
        pad(yearFromNow.getMonth() + 1) +
        "-" +
        pad(yearFromNow.getDate()),
    );
  }
}
      ...
      .ajax({
        ...
        success: function (result) {
          if (result.status === "ok") {
            ...
            console.log(result);
            jQuery(".download-link").html(
              "An unexpected error occurred.<br/>Details have been logged for review.<br/><a href='/help' target='_blank'>Contact the Research Data Service Team</a> with any questions.",
            );
            jQuery("#downloadLinkModal").modal("show");
          }
        },
...
function update_and_publish() {
  jQuery("[id^=edit_dataset]").append(
    "<input type='hidden' name='context' value='publish' />",
  );
  window.onbeforeunload = null;
  jQuery("[id^=edit_dataset]").submit();
}
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      if (data.message === "ok") {
        ...
        jQuery("#validation-warning").html(
          '<div class="alert alert-alert">' + data.message + "</div>",
        );
        jQuery("#update-confirm").prop("disabled", true);
      }
    },
...
function reset_confirm_msg() {
  if (
    jQuery(".publish-msg").html() != undefined &&
    jQuery(".publish-msg").html().length > 0
  ) {
    ...
    jQuery
      .getJSON(
        "/datasets/" +
          dataset_key +
          "/confirmation_message?new_embargo_state=" +
          new_embargo +
          "&release_date=" +
          release_date,
        function (data) {
...
function getNewToken() {
  jQuery.getJSON(
    "/datasets/" + dataset_key + "/get_new_token",
    function (data) {
...
function setTokenExamples(upload_token) {
  jQuery(".current-token").html(
    "<p><strong>Current HTTP Authentication Token: </strong>" + upload_token,
  );
  jQuery("#token-button-text").text("View token for command line tools");
  jQuery(".command-to-copy").html(
    "<pre><code>python databank_api_client_v2.py " +
      dataset_key +
      " " +
      upload_token +
      " myfile.csv</code></pre>",
  );
  jQuery(".curl-to-copy").html(
    "<pre><code>curl -F &quot;binary=@my_datafile.csv&quot; -H &quot;Authorization: Token token=" +
      upload_token +
      "&quot; -H &quot;Transfer-Encoding: chunked&quot; -X POST https://databank.illinois.edu/api/dataset/" +
      dataset_key +
      "/datafile -o output.txt</code></pre>",
  );
}
...
function handleKeywordKeyup() {
  ...
  if (keyword_count > 0) {
    jQuery("#keyword-label").html(
      "Keywords (" + keyword_count + " -- semicolon separated)",
    );
  } else {
    jQuery("#keyword-label").html("Keywords");
    jQuery("#keyword-text").attr(
      "placeholder",
      "[Semicolon separated list of keywords or phrases, e.g.: institutional repositories; file formats]",
    );
  }
}
...
function addReviewerRow() {
  ...
  var reviewerRow =
    "<div class='row'><div class='col-md-1'><div class='pull-right'><input name='reviewer_emails[]' type='checkbox' value='" +
    email +
    "' checked='checked'></div></div><div class='col-md-3'>" +
    email +
    "</div>";
...
function addInternalEditorRow() {
  ...
  var reviewerRow =
    "<div class='row'><div class='col-md-1'><div class='pull-right'><input name='internal_editor[]' type='checkbox' value='" +
    netid +
    "' checked='checked'></div></div><div class='col-md-3'>" +
    netid +
    "</div>";
...
function importFromGlobus() {
  jQuery
    ...
    .done(function (_data, _textStatus, _jqXHR) {
      jQuery("#message").html(
        "<div class='alert alert-alert'><p>Refresh page to see datafiles</p></div>",
      );
    })
    .fail(function (xhr, textStatus, _errorThrown) {
      jQuery("#message").html(
        "<div class='alert alert-alert'><p>Problem ingesting datafiles. " +
          xhr.responseText +
          "</p></div>",
      );
      console.log("error" + textStatus);
