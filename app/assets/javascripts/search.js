    ...
    window.Databank.search = {
        ...
        setSortStyle: function () {
            ...
            switch (sort_criteria) {
                case 'sort_updated_asc':   jQuery('.updated_asc').addClass('btn-current-sort');   break;
                case 'sort_released_asc':  jQuery('.released_asc').addClass('btn-current-sort');  break;
                case 'sort_released_desc': jQuery('.released_desc').addClass('btn-current-sort'); break;
                case 'sort_ingested_asc':  jQuery('.ingested_asc').addClass('btn-current-sort');  break;
                case 'sort_ingested_desc': jQuery('.ingested_desc').addClass('btn-current-sort'); break;
                default:                   jQuery('.updated_desc').addClass('btn-current-sort');
            }
        },
...
function setSortStyle()           { Databank.search.setSortStyle(); }
function handleFilterChange()     { Databank.search.handleFilterChange(); }
function clearFilters()           { Databank.search.clearFilters(); }
function clearSearchTerm()        { Databank.search.clearSearchTerm(); }
function backToSearch()           { Databank.search.backToSearch(); }
function generateReport()         { Databank.search.generateReport(); }
function downloadCitationReport() { Databank.search.downloadCitationReport(); }
function set_per_page()           { Databank.search.handleFilterChange(); }

jQuery(document).ready(function () { Databank.search.init(); });
jQuery(document).on('page:load',   function () { Databank.search.init(); });