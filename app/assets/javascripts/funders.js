...
funders_ready = function () {
  ...
  bindFunderTableEvents();
  handleFunderTable();
  //alert("funders.js javascript working");
};

function bindFunderTableEvents() {
  jQuery("#funder_table")
    .off("click.funderActions")
    .on("click.funderActions", "button[data-funder-action]", function () {
      var action = jQuery(this).data("funder-action");
...
function handleFunderChange(funderIndex) {
  ...
  jQuery("#dataset_funders_attributes_" + funderIndex + "_name").css(
    "visibility",
    "hidden",
  );
  ...
  switch (funderSelectVal) {
    case "none":
      ...
      jQuery(
        "#dataset_funders_attributes_" + funderIndex + "_identifier_scheme",
      ).val("");
      break;
...
function handleFunderTable() {
  var rows = jQuery("#funder_table tr");
  ...
  rows.each(function (i) {
    if (i > 0) {
      ...
      jQuery("td:last-child", this).html(
        funderRowActions(funder_index, i + 1 == rowCount),
      );
    }
  });
}

function funderRowActions(funder_index, isLastRow) {
  return Databank.utils.rowActionButtons({
    includeAdd: isLastRow,
    removeAttributes: {
      "data-funder-action": "remove",
      "data-funder-index": funder_index,
    },
    addAttributes: {
      "data-funder-action": "add",
    },
  });
}
...
function add_funder_row() {
  ...
  var funder_row =
    '<tr class="item row" id="funder_index_' +
    newId +
    '">' +
    "<td>" +
    '<input type="hidden" name="dataset[funders_attributes][' +
    newId +
    '][identifier]" id="dataset_funders_attributes_' +
    newId +
    '_identifier" />' +
    '<input type="hidden" name="dataset[funders_attributes][' +
    newId +
    ...
    newId +
    '_code"><option value="">Please select</option>' +
    ...
    "</td>" +
    "<td>" +
    '<input class="form-control dataset funder-text" placeholder="[Funder Name]" type="text" name="dataset[funders_attributes][' +
    newId +
    '][name]" id="dataset_funders_attributes_' +
    newId +
    '_name" style="visibility: hidden;" />' +
    "</td>" +
    "<td>" +
    '<input class="form-control dataset" type="text" name="dataset[funders_attributes][' +
    newId +
    '][grant]" id="dataset_funders_attributes_' +
    newId +
    '_grant" />' +
    "</td>" +
    "<td></td>" +
...
function remove_funder_row(funder_index) {
  if (
    jQuery("#dataset_funders_attributes_" + funder_index + "_id").val() !=
    undefined
  ) {
    jQuery("#dataset_funders_attributes_" + funder_index + "__destroy").val(
      "true",
    );
    jQuery("#deleted_funder_table > tbody:last-child").append(
      jQuery("#funder_index_" + funder_index),
    );
